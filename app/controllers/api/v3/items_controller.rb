class Api::V3::ItemsController < ApiController
  def index
    ...
    if(params[:sort].present?)
      items = category.items.on_shelf.order(Item.sort_params[params[:sort]]).includes(:on_shelf_specs).select(:id, :name, :price, :special_price, :cover, :slug).page(params[:page]).per_page(20)
    else
