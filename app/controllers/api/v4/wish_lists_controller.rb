class Api::V4::WishListsController < ApiController
  before_action :find_user

  def index
    wish_lists = @user.wish_lists.as_json({only: [:id], include: {item:{only: [:id, :name, :price], methods: [:final_price, :discount_icon_url]}, item_spec: {only: [:id, :style, :style_pic], methods: [:stock_amount]}}})
