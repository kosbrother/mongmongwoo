class Api::V4::CampaignRulesController < ApiController
  ...
  def show
    ...
    items = campaign_rule.items.as_json(only: [:id, :name, :price, :special_price, :cover, :slug], methods: [:discount_icon_url], include: { on_shelf_specs: { only: [:id, :style, :style_pic], methods: [:stock_amount] }})
