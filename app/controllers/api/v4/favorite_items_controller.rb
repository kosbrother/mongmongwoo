class Api::V4::FavoriteItemsController < ApiController
  before_action :find_user

  def index
    favorite_items = @user.favorite_items.as_json({only: [:id], include: {item:{only: [:id, :name, :cover, :price], methods: [:final_price, :discount_icon_url]}}})
