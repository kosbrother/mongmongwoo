class CategoriesController < ApplicationController
  ...
  def show
    ...
    @sort = params[:sort] || @sort_options.first[1]
    @items = @category.items.on_shelf.includes(:campaign_rule).order(Item.sort_params[@sort]).paginate(page: params['page'], per_page: 18)
