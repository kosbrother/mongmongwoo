class Message < ActiveRecord::Base
  ...
  scope :user_messages, -> (user) { joins('LEFT JOIN message_records ON message_records.message_id = messages.id').where('message_type = :message_type OR message_records.user_id = :user_id', message_type: Message.message_types["萌萌屋官方訊息"], user_id: user.id).order(created_at: :DESC) }
