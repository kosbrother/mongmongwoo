    ...
    Item.find_each do |item|
      if(item.created_at < Time.now - 10.days)
        ...
        if m_sales_amount == 0
          ...
          specs_sales = []
          item.specs.each {|spec| specs_sales << OrderItem.product_style_sales(spec.item_id, spec.id).sum_item_quantity}
          ratio = specs_sales.collect{|s| s/(specs_sales.sum.to_f)}
          stock_num = ratio.collect{|r| (r*m_sales_amount).ceil}
          item.specs.each_with_index{|s,i| s.update_attribute(:recommend_stock_num,stock_num[i])}
        end
      end
    end
  end
