namespace :specs do
  ...
  def on_shelf_days(item)
    (Time.current - item.created_at).to_i / (24*60*60)
  end
  ...
  def set_alert_and_recommend_num(item)
    item.specs.on_shelf.each do |spec|
      ...
      sell_days = (end_sell_day - start_sell_day) / (24*60*60)
      alert_num = spec.sales_between(start_sell_day..end_sell_day) / sell_days * 7
      recommend_num = spec.sales_between(start_sell_day..end_sell_day) / sell_days * 30
  ...
  task :stock_recommend_num  => :environment do
    # 建議補貨水位recommend_stock_num : 補到這個水位才夠賣　= 賣的量／（賣的天數) * 30
    # 警示水位alert_stock_num : 這個水位再不補，就會出現缺貨 = 賣的量／（賣的天數) * 7

    # if 上架過
    #   if 上架超過30天
    #     1. 決定是否繼續賣(賣量 < 樣式數 * 2 就recommend_stock_num: 0)
    #     2. 繼續賣,　計算補貨水位，警示水位
    #   elsif 上架超過10天
    #     1. 決定是否繼續賣(賣量 < 樣式數 * 0.5 就recommend_stock_num: 0)
    #     2. 繼續賣,　計算補貨水位，警示水位
    #   elsif 上架超過 5天
    #     1. 決定是否繼續賣(賣量 < 0 就recommend_stock_num: 0)
    #     2. 繼續賣,　計算補貨水位，警示水位
    #   else(即未滿5天)
    #     １. 繼續賣,　計算補貨水位，警示水位
    #   end
    # else
    #   recommend_stock_num: 3
    # end
    ...
    Item.on_shelf.find_each do |item|
      if item.ever_on_shelf
        ...
        item.specs.on_shelf.update_all(recommend_stock_num: 3)
      end
